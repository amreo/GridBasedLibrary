...
''' </summary>
Public NotInheritable Class ManhattanDistanceFunction
    Implements ICoordDistanceFunction

    ''' <summary>
    ''' Ottiene o imposta una istanza di DistanceFunction
    ''' </summary>
    Protected Shared DF As New ManhattanDistanceFunction
    ''' <summary>
    ...
    Public Shared ReadOnly Property DistanceFunction As ManhattanDistanceFunction
        Get
            'restituisce DF
            Return DF
        End Get
    ...
    Function GetDistance(x1 As Integer, y1 As Integer, x2 As Integer, y2 As Integer, Optional scale As Single = 1.0) As Single Implements ICoordDistanceFunction.GetDistance
        'restituisce la distanza di manhattan
        Return scale * (Math.Abs(x1 - x2) + Math.Abs(y1 - y2))
    End Function
    ''' <summary>
    ''' Calcola la distanza tra 1 e 2
    ''' </summary>
    ...
    Function GetDistance(c1 As ILocated, c2 As ILocated, Optional scale As Single = 1.0) As Single Implements ICoordDistanceFunction.GetDistance
        'restituisce la distanza di manhattan
        Return scale * (Math.Abs(c1.GetX - c2.GetX) + Math.Abs(c1.GetY - c2.GetY))
    End Function
End Class
